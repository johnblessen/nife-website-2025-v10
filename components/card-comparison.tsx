...
interface CardComparisonProps {
  onComplete: (plan: any, userData: any) => void;
}

export default function CardComparison({ onComplete }: CardComparisonProps) {
  const [selectedCard, setSelectedCard] = useState("")

  const handleContinue = () => {
    if (selectedCard) {
      const plan = selectedCard === 'kims' ? {
        name: "Kim's Card",
        annualFee: 6999,
        benefits: [
          "₹5L Health Insurance for 2 Adults",
          "RuPay Card with 1% digital gold rewards",
          "Wellness Benefits (Gym, Spa, Salon discounts)",
          "Digital Gold rewards on transactions"
        ]
      } : {
        name: "Health Protect Card",
        annualFee: 3499,
        benefits: [
          "₹3L Health Insurance for Individual",
          "RuPay Card with 1% digital gold rewards",
      ...
      const userData = {
        fullName: "John Doe", // This would come from the previous step
        email: "john@example.com" // This would come from the previous step
      };

      onComplete(plan, userData);
    }
  }
